...
class Seller:
    ...
    def cost_of_reviews(self):
        """
        Returns a DataFrame with:
        'seller_id', 'cost_of_reviews'
        """
        sellers = self.data["sellers"].copy()
        order_items = self.data["order_items"][["seller_id", "order_id"]]
        orders = self.order.get_review_score()

        sellers_orders = (
            sellers.merge(order_items, on="seller_id")
        ...
        sellers_orders["cost_of_reviews"] = sellers_orders["review_score"].map(
            cost_mapping
        )

        return (
            sellers_orders.groupby("seller_id")
            .agg(
                {
                    "cost_of_reviews": "sum",
                }
    ...
    def get_training_data(self):
        ...
        'quantity_per_order', 'sales', 'cost_of_reviews', 'revenues', 'profits']
        """
        ...
        training_set = (
            ...
            .merge(self.cost_of_reviews(), on="seller_id")
