...
class Product:
    ...
    def cost_of_reviews(self):
        """
        Returns a DataFrame with:
        'product_id', 'cost_of_reviews'
        """
        orders_reviews = self.order.get_review_score()
        orders_products = self.data["order_items"][
            ["order_id", "product_id"]
        ].drop_duplicates()
        orders_products_with_score = orders_products.merge(
            orders_reviews, on="order_id"
        )

        cost_mapping = {1: 100, 2: 50, 3: 40, 4: 0, 5: 0}
        orders_products_with_score["review_cost"] = orders_products_with_score[
            "review_score"
        ].map(cost_mapping)
        ...
        result.columns = ["product_id", "cost_of_reviews"]
